            ...
            if (response.status === 200) {
                ...
                for (const meters of Object.keys(objArray)) {
                    ...
                    if (objArray[meters].firstMeasurementTime != -1) {
                        ...
                        await this.setObjectNotExistsAsync(objArray[meters]['serialNumber'], {
                            type: 'device',
                            common: {
                        ...
                        await this.setObjectNotExistsAsync(`${objArray[meters]['serialNumber']}.info`, {
                            type: 'channel',
                            common: {
                                name: 'Information',
                            },
    ...
    async doDiscovergyMeter(endpoint, urlencoded_parameters, meterId) {
        try {
            ...
            if (response.status === 200) {
                ...
                try {
                    this.log.debug(`[doDiscovergyMeter] Data : ${JSON.stringify(response.data)}`);
